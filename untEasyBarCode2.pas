...
type
  ...
  TEasyDataMode = (dmBinary, dmText, dmNumeric);

  { PDF417 types and constants }

  TEasyPDF417CodewordList = array [0..2700] of Word;
  TEasyPDF417ECCLevels = (ecAuto, ecLevel0, ecLevel1, ecLevel2, ecLevel3,
                        ecLevel4, ecLevel5, ecLevel6, ecLevel7, ecLevel8);

  { MaxiCode types and constants }

  TEasyMaxiCodeMode = (cmMode2, cmMode3, cmMode4, cmMode5, cmMode6);

const
  EasyMaxiCodeGaloisField    = 64;  { Galois field to work in }
  EasyMaxiCodeECCPoly        = 67;  { Primary polynomial - }
  EasyMaxMaxiCodeECCDataSize = 144; { Max amount of data }

type
  TEasyMaxiCodeECCData = array [0..EasyMaxMaxiCodeECCDataSize] of Byte;
  TEasyMaxiCodeECCPoly = (epPrimary, epStandard, epEnhanced);
  TEasyMaxiCodeECCInterleave = (imNone, imEven, imOdd);

  { E2DBarcodeError }

  E2DBarcodeError = class (Exception);

  { TEasyCustom2DBarcode }

  TEasyCustom2DBarcode = class (TGraphicControl)
    protected { private }
      FCode              : string;
      FBarWidth          : Integer;
    ...
    public
      ...
      property RelativeBarHeight : Boolean read FRelativeBarHeight write SetRelativeBarHeight
               default False;
  ...
  TEasyPDF417Barcode = class (TEasyCustom2DBarcode)
    private
      FTruncated       : Boolean;
      FCodewords       : TEasyPDF417CodewordList;
    ...
    protected
      ...
      function  GetPDF417ECCLevel : TEasyPDF417ECCLevels;
    ...
    public
      ...
      procedure DrawBarCodePDF417(ACanvas: TCanvas; R: TRect);
    published
      property ECCLevel : TEasyPDF417ECCLevels
  ...
  { TEasyMaxiCodeBarcode }

  TEasyMaxiCodeBarcode = class (TEasyCustom2DBarcode)
    private
      FMode                : TEasyMaxiCodeMode;
      FCodewords           : TEasyMaxiCodeECCData;
      FNumCodewords        : Integer;
      FHighlight           : Boolean;
      FShowCodewords       : Boolean;
      FShowAll             : Boolean;
      FMessage             : TEasyMaxiCodeECCData;
      ...
      FLog     : array [0..EasyMaxiCodeGaloisField] of Integer;
      FAntiLog : array [0..EasyMaxiCodeGaloisField] of Integer;
    ...
    protected
      ...
      procedure SetMode (const v : TEasyMaxiCodeMode);
    ...
    public
      ...
      procedure DrawBarCodeMaxi(ACanvas: TCanvas; R: TRect);
    published
      ...
      property Mode : TEasyMaxiCodeMode
...
procedure Register;
//{$R dcr/untEasyBarCode2.dcr}

implementation
  { PDF417 types and constants }

type
  TEasyPDF417CodewordArray = array [0..2] of array [0..928] of Longint;

const

  StPDF417CellWidth = 17;

  StPDF417Codewords : TEasyPDF417CodewordArray =
...
type
  TEasyPDF417TextCompactionMode = (cmAlpha, cmLower, cmMixed, cmPunctuation,
                                 cmNone);
  TEasyPDF417TextCompactionModes = set of TEasyPDF417TextCompactionMode;

  TEasyPDF417TextCompactionData = record
    Value : Integer;
    Mode : TEasyPDF417TextCompactionModes;
  end;

const
  TEasyPDF417TextCompaction : array [0..127] of TEasyPDF417TextCompactionData =
    ((Value : -1; Mode : []),                                  { 000 }
     (Value : -1; Mode : []),                                  { 001 }
     (Value : -1; Mode : []),                                  { 002 }
     (Value : -1; Mode : []),                                  { 003 }
...
{ TEasyMaxiCode types and constants }

type
  TEasyMaxiCodeCodeSet = (csCodeSetA, csCodeSetB, csCodeSetC, csCodeSetD,
                        csCodeSetE, csNone);

const
  EasyMaxiCodeCodeSets : array [csCodeSetA..csCodeSetE] of
...
procedure Register;
begin
//  RegisterComponents('Easy Extends', [TEasyPDF417Barcode, TEasyMaxiCodeBarcode]);
end;
{ TEasyCustom2DBarcode }

constructor TEasyCustom2DBarcode.Create (AOwner : TComponent);
begin
...
destructor TEasyCustom2DBarcode.Destroy;
begin
  FBitmap.Free;
...
procedure TEasyCustom2DBarcode.CopyToClipboard;
begin
  CopyToClipboardRes (0, 0);
end;

procedure TEasyCustom2DBarcode.CopyToClipboardRes (ResX : Integer;
                                                 ResY : Integer);
var
  MetaFile       : TMetaFile;
...
procedure TEasyCustom2DBarcode.GenerateBarcodeBitmap (BCWidth  : Integer;
                                                    BCHeight : Integer);
var
  TextHeight    : Integer;
  TextWidth     : Integer;
...
function TEasyCustom2DBarcode.GetBarCodeHeight : Integer;
begin
  Result := CalculateBarCodeHeight(Height);
end;

function TEasyCustom2DBarcode.GetBarCodeWidth : Integer;
begin
  Result := CalculateBarCodeWidth(Width);
end;

procedure TEasyCustom2DBarcode.GetCurrentResolution (var ResX : Integer;
                                                   var ResY : Integer);
begin
...
function TEasyCustom2DBarcode.GetVersion : string;
begin
  Result := 'V1.0.0.1';
end;

procedure TEasyCustom2DBarcode.Paint;
begin
...
procedure TEasyCustom2DBarcode.PaintToCanvas (ACanvas  : TCanvas;
                                            Position : TPoint);
begin
  PaintToDC (ACanvas.Handle, Position);
end;

procedure TEasyCustom2DBarcode.PaintToCanvasRes (ACanvas  : TCanvas;
                                               Position : TPoint;
                                               ResX     : Integer;
                                               ResY     : Integer);
begin
...
procedure TEasyCustom2DBarcode.PaintToCanvasSize (ACanvas : TCanvas;
                                                X, Y, H : Double);
var
...
procedure TEasyCustom2DBarcode.PaintToDC (DC : hDC; Position : TPoint);
var
  NewResX : Integer;
  NewResY : Integer;
...
procedure TEasyCustom2DBarcode.PaintToDCRes (DC : hDC; Position : TPoint;
                                           ResX : Integer; ResY : Integer);
var
  ACanvas   : TCanvas;
  R1        : TRect;
...
procedure TEasyCustom2DBarcode.PaintToPrinterCanvas (ACanvas  : TCanvas;
                                                   Position : TPoint);
begin
  PaintToPrinterDC (ACanvas.Handle, Position);
end;

procedure TEasyCustom2DBarcode.PaintToPrinterCanvasRes (ACanvas  : TCanvas;
                                                      Position : TPoint;
                                                      ResX     : Integer;
                                                      ResY     : Integer);
begin
...
procedure TEasyCustom2DBarcode.PaintToPrinterCanvasSize (ACanvas : TCanvas;
                                                       X, Y, H : Double);
var
  PixelsPerInchX : Integer;
  PixelsPerInchY : Integer;
...
procedure TEasyCustom2DBarcode.PaintToPrinterDC (DC : hDC; Position : TPoint);
var
  NewResX : Integer;
  NewResY : Integer;
...
procedure TEasyCustom2DBarcode.PaintToPrinterDCRes (DC       : hDC;
                                                  Position : TPoint;
                                                  ResX     : Integer;
                                                  ResY     : Integer);
var
...
procedure TEasyCustom2DBarcode.SaveToFile (const FileName : string);
begin
  GenerateBarcodeBitmap (Width, Height);
  FBitmap.SaveToFile (FileName);
end;

procedure TEasyCustom2DBarcode.SaveToFileRes (const FileName : string;
                                            ResX : Integer; ResY : Integer);
var
  RenderBmp : TBitmap;
  SizeX     : Integer;
...
procedure TEasyCustom2DBarcode.SetAlignment (const v : TAlignment);
var
...
procedure TEasyCustom2DBarcode.SetBackgroundColor (const v : TColor);
var
...
procedure TEasyCustom2DBarcode.SetBarHeight (const v : Integer);
var
...
procedure TEasyCustom2DBarcode.SetBarHeightToWidth (const v : Integer);
var
...
procedure TEasyCustom2DBarcode.SetBarWidth (const v : Integer);
var
  OldBarWidth : Integer;
...
procedure TEasyCustom2DBarcode.SetBitmap (const v : TBitmap);
begin
  FBitmap.Assign (v);
  Invalidate;
end;

procedure TEasyCustom2DBarcode.SetCaption (const v : string);
var
  OldCaption : string;
...
procedure TEasyCustom2DBarcode.SetCaptionLayout (const v : TTextLayout);
var
...
procedure TEasyCustom2DBarcode.SetCode (const v : string);
var
  OldCode : string;
...
procedure TEasyCustom2DBarcode.SetECCLevel (const v : Integer);
var
  OldECCLevel : Integer;
...
procedure TEasyCustom2DBarcode.SetExtendedSyntax (const v : Boolean);
var
...
procedure TEasyCustom2DBarcode.SetQuietZone (const v : Integer);
var
...
procedure TEasyCustom2DBarcode.SetRelativeBarHeight (const v : Boolean);
var
...
procedure TEasyCustom2DBarcode.SetVersion(const Value : string);
begin
end;

{ TEasyPDF417Barcode }

procedure TEasyPDF417Barcode.DrawBarcodePDF417(ACanvas: TCanvas; R: TRect);
begin
  GenerateBarcodeBitmap (Width, Height);
  ACanvas.CopyRect(R, FBitmap.Canvas, Rect(0, 0, FBitmap.Width, FBitmap.Height));
  FBitmap.SaveToFile('c:\pdf417.bmp');
//  FBitmap.SaveToFile (FileName);
end;

constructor TEasyPDF417Barcode.Create (AOwner : TComponent);
begin
  inherited Create (AOwner);
...
procedure TEasyPDF417Barcode.AddCodeword (Value : Word);
begin
  FCodewords[FNumCodewords] := Value;
  Inc (FNumCodewords);
end;

function TEasyPDF417Barcode.CalculateBarCodeWidth (
...
function TEasyPDF417Barcode.CalculateBarCodeHeight (
...
procedure TEasyPDF417Barcode.CalculateECC (NumCodewords : Integer;
                                         ECCLen       : Integer);
...
procedure TEasyPDF417Barcode.CalculateSize (var XSize : Integer;
                                          var YSize : Integer);
var
  i                 : Integer;
...
function TEasyPDF417Barcode.CodewordToBitmask (RowNumber : Integer;
...
procedure TEasyPDF417Barcode.ConvertBytesToBase900 (const S : array of Byte;
...
procedure TEasyPDF417Barcode.ConvertToBase900 (const S  : string;
                                             var A    : array of Integer;
                                             var LenA : Integer);
var
  D          : string;
...
procedure TEasyPDF417Barcode.DrawBarcode;
var
  XSize             : Integer;
  YSize             : Integer;
  i                 : Integer;
...
procedure TEasyPDF417Barcode.DrawCodeword (RowNumber     : Integer;
                                         ColNumber     : Integer;
                                         WorkBarHeight : Integer;
                                         Pattern       : string);
...
procedure TEasyPDF417Barcode.DrawCodewordBitmask (RowNumber     : Integer;
                                                ColNumber     : Integer;
                                                WorkBarHeight : Integer;
                                                Bitmask       : DWord);
...
procedure TEasyPDF417Barcode.DrawLeftRowIndicator (RowNumber     : Integer;
                                                 WorkBarHeight : Integer;
                                                 NumRows       : Integer;
                                                 NumCols       : Integer);
var
...
procedure TEasyPDF417Barcode.DrawRightRowIndicator (RowNumber     : Integer;
                                                  ColNumber     : Integer;
                                                  WorkBarHeight : Integer;
                                                  NumRows       : Integer;
                                                  NumCols       : Integer);
...
procedure TEasyPDF417Barcode.DrawStartPattern (RowNumber     : Integer;
                                             WorkBarHeight : Integer);
begin
  DrawCodeword (RowNumber, 0, WorkBarHeight, '81111113');
end;

procedure TEasyPDF417Barcode.DrawStopPattern (RowNumber     : Integer;
                                            ColNumber     : Integer;
                                            WorkBarHeight : Integer);
begin
  if Truncated then
...
procedure TEasyPDF417Barcode.EncodeBinary (var Position : Integer;
                                         CodeLen      : Integer);
...
procedure TEasyPDF417Barcode.EncodeNumeric (var Position : Integer;
                                          CodeLen      : Integer);
...
procedure TEasyPDF417Barcode.EncodeText (var Position : Integer;
                                       CodeLen      : Integer);

  function SelectBestTextMode (
        CurChar : TEasyPDF417TextCompactionData) : TEasyPDF417TextCompactionMode;
  begin
  ...
  function ChangeTextSubmode (CurrentMode : TEasyPDF417TextCompactionMode;
                              NewMode : TEasyPDF417TextCompactionMode;
                              UseShift : Boolean) : TEasyPDF417TextCompactionMode;
  const
...
var
  CurrentTextSubmode : TEasyPDF417TextCompactionMode;
  CurChar            : TEasyPDF417TextCompactionData;
  UseShift           : Boolean;
  Done               : Boolean;
  Dummy              : Integer;
  NewChar            : Integer;
...
begin
  ...
  while (Position <= CodeLen) and (CurChar.Value >= 0) and
    ...
    if (Position <= CodeLen) then begin
      ...
      CurChar := TEasyPDF417TextCompaction[NewChar];
    end;
      ...
      if not (CurrentTextSubmode in CurChar.Mode) then begin
        ...
        if Position < CodeLen then begin
          if not (CurrentTextSubmode in
             TEasyPDF417TextCompaction[Integer (Code[Position + 1])].Mode) then
            UseShift := False
          else
            UseShift := True;
        end else
...
procedure TEasyPDF417Barcode.GenerateCodewords;
var
  ErrorLevel        : Integer;
  NumErrorCodewords : Integer;
  XSize             : Integer;
...
procedure TEasyPDF417Barcode.GetNextCharacter (var NewChar  : Integer;
                                             var Codeword : Boolean;
                                             var Position : Integer;
                                             CodeLen      : Integer);
var
...
function TEasyPDF417Barcode.GetPDF417ECCLevel : TEasyPDF417ECCLevels;
begin
  case FECCLevel of
    0 : Result := ecLevel0;
    1 : Result := ecLevel1;
...
function TEasyPDF417Barcode.GetRealErrorLevel : Integer;
begin
...
function TEasyPDF417Barcode.GoodForNumericCompaction (
                                               Position  : Integer;
                                               CodeLen   : Integer;
                                               var Count : Integer) : Boolean;
const
...
function TEasyPDF417Barcode.GoodForTextCompaction (
                                             Position  : Integer;
                                             CodeLen   : Integer;
                                             var Count : Integer) : Boolean;
  ...
  begin                                                                {!!.01}
    if v > #127 then                                                   {!!.01}
      Result := False                                                  {!!.01}
    else if TEasyPDF417TextCompaction[Integer (v)].Value >= 0 then       {!!.01}
      Result := True                                                   {!!.01}
    else                                                               {!!.01}
      Result := False;                                                 {!!.01}
  end;                                                                 {!!.01}
...
procedure TEasyPDF417Barcode.RenderToResolution (var OutBitmap : TBitmap;
                                               ResX          : Integer;
                                               ResY          : Integer;
                                               var SizeX     : Integer;
                                               var SizeY     : Integer);
...
procedure TEasyPDF417Barcode.SetBarHeight (const v : Integer);
begin
...
procedure TEasyPDF417Barcode.SetBarHeightToWidth (const v : Integer);
begin
...
procedure TEasyPDF417Barcode.SetBarWidth (const v : Integer);
begin
  if v < 1 then
...
procedure TEasyPDF417Barcode.SetPDF417ECCLevel (const v : TEasyPDF417ECCLevels);
var
  NewLevel : Integer;
  OldLevel : Integer;
...
procedure TEasyPDF417Barcode.SetRelativeBarHeight (const v : Boolean); 
begin
...
procedure TEasyPDF417Barcode.SetTruncated (const v : Boolean);
var
...
function TEasyPDF417Barcode.IsNumericString (const S : string) : boolean;
var
  i      : integer;
  LenS : integer;
...
procedure TEasyPDF417Barcode.SetNumColumns (const v : Integer);
var
...
procedure TEasyPDF417Barcode.SetNumRows (const v : Integer);
var
  OldNumRows : Integer;
...
procedure TEasyPDF417Barcode.TextToCodewords;
var
  i                  : Integer;
  CodeLen            : Integer;
  CurrentMode        : TEasyDataMode;
  Count              : Integer;
  First              : Boolean;
...
procedure TEasyPDF417Barcode.DrawPDF417Barcode(Canvas: TCanvas);
var
  XSize             : Integer;
  YSize             : Integer;
  i                 : Integer;
...
constructor TEasyMaxiCodeBarcode.Create (AOwner : TComponent);
begin
  inherited Create (AOwner);
...
procedure TEasyMaxiCodeBarcode.AddCodeword (Value : Integer);
begin
...
function TEasyMaxiCodeBarcode.CalculateBarCodeWidth (
                                        PaintableWidth : Integer) : Integer;
begin
  Result := Round (30 * FMaxiHexWidth + FMaxiHexHOffset);
end;

function TEasyMaxiCodeBarcode.CalculateBarCodeHeight (
                                        PaintableHeight : Integer) : Integer;
begin
  Result := Round (33 * FMaxiHexHeight + 33 * FMaxiHexVOffset);
end;

procedure TEasyMaxiCodeBarcode.DrawBarcode;
...
procedure TEasyMaxiCodeBarcode.DrawFinder;
var
  CenterX : Integer;
  CenterY : Integer;
...
procedure TEasyMaxiCodeBarcode.DrawHex (XPos, YPos : Integer);
var
  XOffset   : Integer;
  YOffset   : Integer;
  HexWidth  : Integer;
...
procedure TEasyMaxiCodeBarcode.GenerateCodewords;
begin
  TextToCodewords;
end;

procedure TEasyMaxiCodeBarcode.GenerateECC;
 { Calculate the ECC codes for MaxiCode }
  ...
  begin
    if (a = 0) or (b = 0) then
      ...
      Result := FAntiLog[(FLog[a] + FLog[b]) mod (EasyMaxiCodeGaloisField - 1)];
  end;
  ...
  begin
    if b = 0 then
      Result := 1 - EasyMaxiCodeGaloisField
    else if a = 0 then
      Result := 0
    else
      Result := FAntiLog[(FLog[a] - FLog[b] +
                (EasyMaxiCodeGaloisField - 1)) mod (EasyMaxiCodeGaloisField - 1)];
  end;

  procedure FillLogArrays (EasyMaxiCodeGaloisField : Integer;
                           EasyMaxiCodeECCPoly : Integer);
  { Populate the log and antilog tables for Galois field arithmetic }
  var
    i : Integer;

  begin
    FLog[0] := 1 - EasyMaxiCodeGaloisField;
    FAntiLog[0] := 1;
    for i := 1 to EasyMaxiCodeGaloisField - 1 do begin
      FAntiLog[i] := FAntiLog[i - 1] * 2;
      if FAntiLog[i] >= EasyMaxiCodeGaloisField then
        FAntiLog[i] := FAntiLog[i] xor EasyMaxiCodeECCPoly;
      FLog[FAntiLog[i]] := i;
    end;
  end;

  procedure CalculateECCCodes (var Data   : TEasyMaxiCodeECCData;
                               Polynomial : TEasyMaxiCodeECCPoly;
                               IStart     : TEasyMaxiCodeECCInterleave);
  ...
  var
    BRegisters   : TEasyMaxiCodeECCData; { Works space for calculating RS ECC }
  ...
  begin
    ...
    for i := 0 to EasyMaxMaxiCodeECCDataSize do
      BRegisters[i] := 0;

    { Calculate the Log and AntiLog tables }
    FillLogArrays (EasyMaxiCodeGaloisField, EasyMaxiCodeECCPoly);
...
procedure TEasyMaxiCodeBarcode.GetNextCharacter (var NewChar  : Integer;
                                               var Codeword : Boolean;
                                               var Position : Integer;
                                               CodeLen      : Integer);
var
...
procedure TEasyMaxiCodeBarcode.GetSizes;
var
  ResX : Integer;
  ResY : Integer;
...
procedure TEasyMaxiCodeBarcode.GetSizesEx (ResX : Integer; ResY : Integer);
begin
...
procedure TEasyMaxiCodeBarcode.PlotCell (Row : Integer; Col : Integer);
var
  XPos : Integer;
  YPos : Integer;
...
procedure TEasyMaxiCodeBarcode.RenderToResolution (var OutBitmap : TBitmap;
                                                 ResX          : Integer;
                                                 ResY          : Integer;
                                                 var SizeX     : Integer;
                                                 var SizeY     : Integer);
...
procedure TEasyMaxiCodeBarcode.SetAutoScale (const v : Boolean);
var
...
procedure TEasyMaxiCodeBarcode.SetBarHeight (const v : Integer);
begin
...
procedure TEasyMaxiCodeBarcode.SetBarWidth (const v : Integer);
begin
...
procedure TEasyMaxiCodeBarcode.SetCarrierCountryCode (const v : Integer);
var
...
procedure TEasyMaxiCodeBarcode.SetCarrierPostalCode (const v : string);
var
...
procedure TEasyMaxiCodeBarcode.SetCarrierServiceClass (const v : Integer);
var
...
procedure TEasyMaxiCodeBarcode.SetHorPixelsPerMM (const v : Extended);
var
  OldHorPixelsPerMM : Extended;
...
procedure TEasyMaxiCodeBarcode.SetMode (const v : TEasyMaxiCodeMode);
var
  OldMode : TEasyMaxiCodeMode;
...
procedure TEasyMaxiCodeBarcode.SetVerPixelsPerMM (const v : Extended);
var
  OldVerPixelsPerMM : Extended;
...
procedure TEasyMaxiCodeBarcode.TextToCodewords;


  function FindCodeSet (Value : Char) : TEasyMaxiCodeCodeSet;
  begin
    Result := csCodeSetA;
    while Result < csNone do begin
      if EasyMaxiCodeCodeSets[Result][Integer (Value)] <> -1 then
        Exit; 
      Inc (Result);
    end;
    Result := csNone;
  ...
  function ChangeCodeSet (CurrentMode   : TEasyMaxiCodeCodeSet;
                          NewMode       : TEasyMaxiCodeCodeSet;
                          Value         : Char;
                          UseShift      : Boolean;
                          UseTwoShift   : Boolean;
                          UseThreeShift : Boolean) : TEasyMaxiCodeCodeSet;
  const
    ShiftAB  = 59;
    ShiftAC  = 60;
    ShiftAD  = 61;
  ...
  var
    CodeLen     : Integer;
    CurrentMode : TEasyMaxiCodeCodeSet;
    UseShift    : Boolean;
    UseShift2   : Boolean;
    UseShift3   : Boolean;
    WorkMode    : TEasyMaxiCodeCodeSet;
    i           : Integer;
    Codeword    : Boolean;
    NewChar     : Integer;
  ...
  begin
    ...
    while i <= CodeLen do begin
      ...
      if CodeWord then
        AddCodeword (NewChar)
      else if EasyMaxiCodeCodeSets[CurrentMode][NewChar] = -1 then begin
        WorkMode := FindCodeSet (Char (NewChar));
        UseShift := False;
        UseShift2 := False;
        UseShift3 := False;
        if i < CodeLen then begin
          if EasyMaxiCodeCodeSets[CurrentMode][Integer (Code[i + 1])] <> -1 then
            UseShift := True;
        end;
        CurrentMode := ChangeCodeSet (CurrentMode, WorkMode, Char (NewChar),
                                      UseShift, UseShift2, UseShift3);
        AddCodeword (EasyMaxiCodeCodeSets[WorkMode][NewChar]);
      end else
        AddCodeword (EasyMaxiCodeCodeSets[CurrentMode][NewChar]);
    end;
  ...
  begin
    ...
    if FCodewords[0] = $02 then begin
      ...
      for i := 0 to 5 do begin
        WorkNum := EasyMaxiCodeCodeSets[csCodeSetA][Integer (WorkStr[6 - i])];
        if WorkNum < 0 then
          WorkNum := EasyMaxiCodeCodeSets[csCodeSetA][32]; { Use a space }
...
procedure TEasyMaxiCodeBarcode.DrawBarCodeMaxi(ACanvas: TCanvas; R: TRect);
begin
  GenerateBarcodeBitmap (Width, Height);
  ACanvas.CopyRect(R, FBitmap.Canvas, Rect(0, 0, FBitmap.Width, FBitmap.Height));
  FBitmap.SaveToFile('c:\Maxi.bmp');
end;
